...
List<Landmark> get favoriteLandmarks =>
    _landmarkData.where((l) => l.isFavorite).toList();
...
Future<Null> landmarksData() async {
  _landmarkData = await _load(
      'landmarkData.json',
      (data) => List.unmodifiable(
          (data).map((element) => Landmark.fromJSON(element))));
}
